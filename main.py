...
def view_lessons():
    if request.method == 'POST':
        lesson_id = request.form.get('lesson_id')  # รับค่า lesson_id จาก form
        if lesson_id:  # ตรวจสอบว่ามีการเลือก lesson_id หรือไม่
            lessons = get_lessons_by_id(lesson_id)
        else:
...
def get_lessons_by_id(lesson_id):
    connection = sqlite3.connect('instance/questions.db')
    cursor = connection.cursor()
    cursor.execute('SELECT * FROM lesson WHERE lesson_id = ?', (lesson_id,))
    lessons = cursor.fetchall()
...
def edit_lesson(lesson_id):
    ...
    if request.method == 'POST':
        ...
        if new_image:
            lesson.image_filename = '/uploads/' + save_uploaded_file(new_image, lesson.image_filename)

        for sound_type in ['thai', 'hmong_daw', 'hmong_njua', 'lahu', 'lisu', 'arka', 'karen']:
            sound_file = request.files[f'new_sound_{sound_type}']
            if sound_file:
                setattr(lesson, f'sound_filename_{sound_type}', '/uploads/' + save_uploaded_file(sound_file, getattr(lesson, f'sound_filename_{sound_type}')))
...
def save_uploaded_file(file, existing_filename):
    if file:
        filename = secure_filename(file.filename)
        file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
        if existing_filename:
            delete_existing_file(existing_filename)
        return filename
...
def view_questions():
    selected_lesson = 1
    questions = Question.query.filter_by(lesson=selected_lesson).all()
...
def edit_question(question_id):
    ...
    if question is None:
        return "ไม่มีคำถามนี้"

    if request.method == 'POST':
        question.question = request.form['question']
        question.lesson = request.form['lesson']

        new_image = request.files['new_image']
        if new_image:
            filename = secure_filename(new_image.filename)
            new_image.save(os.path.join('static/uploads', filename))
            question.image = 'uploads/' + filename

        db.session.commit()
        flash('คำถามถูกแก้ไขเรียบร้อยแล้ว', 'success')
        return redirect(url_for('teacher_dashboard'))
